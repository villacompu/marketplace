...
ANALYTICS_LOCK = os.path.join(DATA_DIR, "analytics.json.lock")
...
def default_db() -> Dict[str, Any]:
    return {
        "meta": {"version": 1, "created_at": now_iso()},
        "users": [],
        "profiles": [],
        "products": [],
        "favorites": [],
...
def load_db() -> Dict[str, Any]:
    ...
    # Compatibilidad por si el archivo viejo trae events
    db.setdefault("meta", {"version": 1, "created_at": now_iso()})
    db.setdefault("users", [])
...
def save_db(db: Dict[str, Any]) -> None:
    ...
    # Seguridad: no guardar analytics aquí
    db = dict(db)
    db.pop("events", None)
...
def load_analytics() -> Dict[str, Any]:
    ...
    analytics.setdefault("meta", {"version": 1, "created_at": now_iso()})
...
def user_profile(db: Dict[str, Any], owner_user_id: str) -> Optional[Dict[str, Any]]:
    for p in db["profiles"]:
        if p.get("owner_user_id") == owner_user_id:
            return p
    return None
...
def seed_if_empty(db: Dict[str, Any]) -> Dict[str, Any]:
    ...
    demo_products = [
        {
            "name": "Caja de brownies x6",
            "description": "Brownies húmedos con chocolate premium. Ideal para regalo.",
            "price_type": "FIXED",
            "price_value": 32000,
            "category": "Comida",
            "subcategory": "Postres",
            "tags": ["brownie", "regalo", "chocolate"],
        },
        {
            "name": "Café molido 250g",
            "description": "Café de origen, tueste medio, notas a cacao y caramelo.",
            "price_type": "FROM",
            "price_value": 28000,
            "category": "Bebidas",
            "subcategory": "Café",
            "tags": ["café", "origen", "tostado"],
        },
        {
            "name": "Catering para eventos",
            "description": "Servicio a convenir según cantidad de personas y menú.",
            "price_type": "AGREE",
            "price_value": None,
            "category": "Servicios",
            "subcategory": "Eventos",
